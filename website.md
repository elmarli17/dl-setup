...
完善现有的多分类方法，增加数据集种类，正常人脸和自然物体数据集，和现有的7类疾病图像一起共9种分类，使用现有尺寸（896*896）完成训练
在1的基础上，寻找合适的GPU平台，使用更大尺寸图像训练，以提高精度。
改善分类精度，使用2分类法。在验证2分类能够改善精度的基础上，使用更大尺寸的输入图像，完成验证。具体尺寸受限于训练平台的显存
现有数据集图像和未来生产环境图像差异的处理。数据集图像为单反室内闪光环境拍摄，生产环境为手机摄像头室内外自然环境的拍摄，若没有预处理则预测结果存在巨大误差。
所以需要对现在图像进行预处理。
人脸图像数据的脱敏（去隐私），若用于外部高校合作以及利用公共GPU平台进行训练，需要寻找适合用于神经网路训练的人脸图像数据脱敏的方法。
 或者用传统的图形处理方法，去除面部五官，达到去隐私的目的。
病变部位图像定位和分割（为了最终具备好的用户体验，需要把面部病变部分定位和分割出来，目前面对的难点是病变类型多种，位置也是多种多样，具备一定技术难度）
二维人脸图像的三维化处理（可选项目不一定实施，和高校合作完成，用于提升6中面积计算的精度）
生产环境下提升预测的应用后台的计算性能，目前GPU测试一张图片需要0.5秒左右，CPU测试一张图片需要3秒左右，需要优化；
所有图像的后处理，在深度学习基础上，后面可以寻找引入专家经验的方法，提升分类的精度。
